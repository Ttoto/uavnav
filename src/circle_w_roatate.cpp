...
#define CENTER_Y (0.4)
        ...
        if(mission_state==CICLE)
        {
            ...
            static double rot_speed_deg;
	    static double rot_speed_rad;
		...
		once=1;
                ...
                rot_speed_deg = 360/one_circle_time;
                est_time = 2*one_circle_time;
                init_q = Eigen::Quaterniond(1,0,0,0);
		rot_speed_rad = (2*PI)/one_circle_time;
                cout << "est time: " << est_time << endl;
            }
            pose.pose.position.x = (CENTER_X + RADIUS * cos (secs*rot_speed_deg * PI/180));
            pose.pose.position.y = (CENTER_Y + RADIUS * sin (secs*rot_speed_deg * PI/180));

            Eigen::Vector3d rpy= euler_from_rotation_matrix(Matrix3d(init_q));
            rpy[2] += secs*rot_speed_rad;
            //            while(1){
            //                if(rpy[2]>=(2*PI))
            //                {
            //                    rpy[2]-=(2*PI);
